...
{
    qWarning() << "State Change : " + QString::number(m_client->state());
}
...
void QGCApplication::setMqttHost(QString host)
{
    ...
    m_client->setHostname(mqttHost);
    if(connectionAttempts > 3){
        qWarning() << "Trying new connection to " << mqttHost << " try " << connectionAttempts;
        m_client->connectToHost();
    }
    connectionAttempts = 0;
}
...
void QGCApplication::updateMessage(const QMqttMessage &msg)
{
    ...
    switch (commandsList.indexOf(message["instruction"].toString())){
        ...
        case 2:
            qWarning() << "=================================================";
            qWarning() << "recieved RESET_GIMBAL";
            qWarning() << "=================================================";
            if(!requestVehicle) {
        ...
        case 5:
            qWarning() << "=================================================";
            qWarning() << "recieved SET_CAMERA";
            qWarning() << "=================================================";
            state_value = 0;
            break;
        case 6:
            qWarning() << "=================================================";
            qWarning() << "recieved SET_CAMERA_INTRINSICS";
            qWarning() << "=================================================";
            state_value = 0;
            break;
        case 7:
            qWarning() << "=================================================";
            qWarning() << "recieved GET_CAMERA";
            qWarning() << "=================================================";
            if(!requestVehicle) {
                qWarning() << "*****   No vehicle available   *****";
                break;
        ...
        case 9:
            qWarning() << "=================================================";
            qWarning() << "recieved TAKE_PHOTO";
            qWarning() << "=================================================";
            state_value = QGCApplication::takePhoto();
            break;
        case 10:
            qWarning() << "=================================================";
            qWarning() << "recieved START_RECORDING";
            qWarning() << "=================================================";
            state_value = QGCApplication::startRecording();
            break;
        case 11:
            qWarning() << "=================================================";
            qWarning() << "recieved STOP_RECORDING";
            qWarning() << "=================================================";
            state_value = QGCApplication::stopRecording();
            break;
        case 12:
            qWarning() << "=================================================";
            qWarning() << "recieved MAV_CMD_DO_SET_SERVO";
            qWarning() << "=================================================";
            if(!requestVehicle) {
                qWarning() << "*****   No vehicle available   *****";
                break;
        ...
        case 14:
            ...
            qWarning() << "=================================================";
            {
                ...
                requestVehicle->guidedModeTakeoff(takeOfHeight ? takeOfHeight : 4); // default takeoff height is 3m
                state_value = -2;
            }
        ...
        case 18:
            ...
            message.insert("horizontalSpeed", t_hSpeed);
            message.insert("verticalSpeed", t_vSpeed);
            tAttitude.insert("yaw", t_yaw);
            tAttitude.insert("pitch", t_pitch);
            tAttitude.insert("roll", t_roll);
            message.insert("attitude", tAttitude);
            state_value = 0;
            break;
        case 19:
            qWarning() << "=================================================";
            qWarning() << "recieved GO_TO_WAYPOINT";
            qWarning() << "=================================================";
            if(!requestVehicle) {
                qWarning() << "*****   No vehicle available   *****";
                break;
        ...
        case 29:
            ...
            QString mission = QString::fromStdString(message["mission"].toString());
...
void QGCApplication::updateStatus(QMqttSubscription::SubscriptionState state)
{
    ...
    case QMqttSubscription::UnsubscriptionPending:
        qWarning() << "Pending Unsubscription";
        break;
    default:
        qWarning() << "--Unknown--";
        break;
...
void QGCApplication::sendAircraftPositionInfo() {
    ...
    for(int i=0; i<vehicles->count(); i++){
        ...
        if(hasGimbal) {
            ...
            if(currentGimbal) {
        //        qWarning() << "============== current gimbal values ==============";
...
bool QGCApplication::busProcessMsg(GstElement *pipeline, GstMessage *msg, QString prefix)
{
    ...
    switch (mType) {
        case (GST_MESSAGE_ERROR):
            ...
            if(err) {
                qWarning() << "ERR = " << err->message << " FROM " << GST_OBJECT_NAME(msg->src);
                g_clear_error(&err);
            } else {
                qWarning() << "NO ERR";
            }
...
void QGCApplication::codeThreadBus(GstElement *pipeline, GoblinData &data, QString prefix)
{
    ...
    qWarning() << "BUS THREAD FINISHED : " << prefix;
}
...
int QGCApplication::takePhoto()
{
    ...
    _activeCamera->setCameraModePhoto();
    _activeCamera->takePhoto(); */
    ...
    qWarning() << "==============   TAKE_PHOTO   =============="; // NEED TO UPDATE FOR OTHER CAMS
}
...
int QGCApplication::startRecording()
{
    ...
    qWarning() << this->isRecording;
    return this->isRecording ? 0 : -1;
    qWarning() << "==============   START_RECORDING   =============="; // NEED TO UPDATE FOR OTHER CAMS
}
...
bool QGCApplication::isFileEmpty(const std::string& filePath)
{
    ...
    if (!file.is_open()) {
        qWarning() << "Erreur : Impossible d'ouvrir le fichier.";
