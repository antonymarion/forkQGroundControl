...
void QGCApplication::startStream(){
    ...
    std::thread threadBus([this]() -> void {
        const gchar* pipelineDesc = "rtspsrc location=rtsp://localhost:8554/city-traffic is-live=true name=src ! rtph264depay ! h264parse ! flvmux streamable=true ! rtmpsink location=rtmp://ome.stationdrone.net/app/city-traffic8 live=true name=sink";
        GError *err = nullptr;
